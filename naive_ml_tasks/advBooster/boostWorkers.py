...
### ---------------------- simple early stop ---------------------- ####
def _singleEarlyStopping(trainData,validData,orgSet):
    earlyStopParamListDict = {
        "objective": "multiclass",
        "boosting": ["gbdt"],
        "num_class": 15,
        ...
        'max_depth': -1,  # -1 means no limit
        # 'min_child_samples': 20,  # Minimum number of data need in a child(min_data_in_leaf)
        'max_bin': 3,  # Number of bucketed bin for feature values
        'nthread': 4,
    }

    evals_results = {}
    x_train,y_train,x_valid,y_valid = orgSet
    print (earlyStopParamListDict)
    lgbm = lgb.train(earlyStopParamListDict,
                    trainData,
...
def singleTraining(trainList,validList):
    print ("")
    ...
    lgbm = _singleEarlyStopping(trainData,validData,[nx,ny,nvx,nvy])
    return lgbm

def lgbBoosterEearlyStopping(trainingSet,testingSet,serializer,modelName):
    train_x,train_y = trainingSet
    test_x ,test_y = testingSet
